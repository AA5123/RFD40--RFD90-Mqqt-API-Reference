...
export default function StatusCodes({ responses }) {
  if (!responses || Object.keys(responses).length === 0) {
    return null;
  }

  const firstCode = Object.keys(responses)[0];
  const responseItem = responses[firstCode] || {};
  const content = responseItem.content || {};
  const mimeType = Object.keys(content)[0];
  const contentItem = mimeType ? content[mimeType] : {};
  const schema = contentItem?.schema || {};
  ...
  return (
    <section className="payload-card payload-card--response">
      <h3 className="payload-card__title">MQTT RESPONSE PAYLOAD</h3>
      <div className="payload-card__tabs" role="tablist" aria-label="Response payload tabs">
        {examples.length > 0 && (
          <button
            type="button"
